<!--
TITLE: The Anatomy of a Token Bill
SUBTITLE: A field guide to context bloat in AI coding agents — and what people have built to fight it.
...
I've been using Claude Code and Cursor daily for months, and like most people I add MCP servers the way you add browser extensions: something looks useful, you connect it, you move on. I'd never actually sat down and measured what that habit costs per turn. So I did. Four scenarios, same local environment, same two questions, MCP servers on and off. Here's exactly where it all went.

None of this is really about Cursor. AI coding agents carry large amounts of context from turn to turn — Cursor just happens to expose an itemized breakdown, which is the only reason I could see where the money went. And MCP servers, which I assumed were the main culprit, turned out to be a real but comparatively modest charge on a much larger bill: scaffolding I never see, tool definitions I may never use, and conversation history that keeps growing the moment the agent starts acting. Treat what follows as one readable meter for a broader problem in agentic coding tools.

## The four-scenario experiment

I ran this in Cursor because it exposes the numbers. The same budget pressure shows up across AI coding agents, even when the meter is hidden.

The setup was boring on purpose: same local coding environment, same 200K context window, same two prompts every time — a throwaway question, then a follow-up asking the agent to double-check itself. The only thing I changed was whether my MCP servers were on or off. Boring is what you want when you're trying to isolate one variable.

One local coding environment, one setup, one run per scenario. Treat this as a field measurement, not a benchmark. The exact numbers will vary; the shape of the bill is the point.
...
**Scenario 1b — same setup, then "are you double sure, please check again."** Total climbed, and here's the useful part: every scaffolding line stayed exactly the same token-for-token — system prompt, tool definitions, rules, skills, subagents, all frozen. The only thing that moved was the conversation. My seven-word follow-up, plus the agent's re-verification and its reply, quadrupled the conversation cost. A polite double-check is not free, and it's not close to free.

**Scenario 2 — fresh session, all MCP servers on, same first question.** Total: ~31.9K. My 18 MCP tools added about 6.3K tokens of schemas, paid every turn whether I use them or not. What I didn't expect: turning MCPs on nudged almost everything else up slightly too — system prompt, built-in tool definitions, rules, skills, subagents, all a little larger. Small individually, but it told me the scaffolding isn't as modular as I assumed — wiring in MCP support seems to touch glue text across categories that have nothing to do with MCP on paper.

**Scenario 2b — same MCP setup, same follow-up question.** This is the one that made me stop. Total: ~60.4K. The scaffolding didn't move — same frozen baseline as Scenario 2. Conversation alone went from a handful of tokens to ~29K.

But the fairest comparison isn't the conversation total. It's the follow-up turn itself: those same three words cost 3,292 tokens without MCPs and 24,708 with them.

Roughly 7x. Same three words. Because this time the agent didn't just reply — it went back and re-queried its tools, and all of that flooded into the conversation.
...
![Four scenarios, one local environment, one variable: MCP servers on or off](figures/chart-a.png)
...
So here's the shape of it, in plain terms: most of what fills your context window on turn one isn't your prompt, it's your agent — Claude Code, Cursor, or whatever you're running — re-explaining itself to the model. MCP servers add a real but comparatively modest tax to that baseline. And then the conversation itself, especially once tool calls start producing real output, becomes the thing that actually eats the window.

Tool results and model replies don't evaporate after they're generated. They sit in the conversation, and the conversation gets re-read, in full, every single turn. Two turns took me to 30% of a 200K window. I don't want to know what a full afternoon of real debugging looks like — actually, I do, and I'm a little afraid to check.

One honest caveat before I get to the fixes: my MCP tax (6.3K for 18 tools) is much gentler than the horror stories floating around online, where people report 40-tool stacks eating 143K of a 200K window on schemas alone. Two likely reasons.
...
Once you look at the problem as a bill, the fixes become easier to sort. Some try to shrink the tool-schema line item. Some stop raw command output before it floods the conversation. Some compress bulky context after it has already been produced. Some trim the model's own reasoning. And some try to prevent the agent from doing unnecessary work in the first place.
...
What's eating your context window? Pull up whatever your agent exposes about token usage — I'll bet the number is higher than you think. I'd genuinely like to know what you find.
