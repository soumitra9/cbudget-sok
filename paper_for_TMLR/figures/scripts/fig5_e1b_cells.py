"""Fig. 5: E2 interaction plot (RTK x CoD, GT outcome)."""
...
def main() -> None:
    apply_style()
    analysis = json.loads((RESULTS_ROOT / "e1b" / "analysis.json").read_text(encoding="utf-8"))
    boot = analysis["raw_interaction_bootstrap"]
    rows = load_run_summary("e1b")

    cell_values: dict[str, list[float]] = {}
    for row in rows:
        key = f"Y{int(row['rtk_on'])}{int(row['factor_b_on'])}"
        cell_values.setdefault(key, []).append(float(row["gt"]))
    ...
    ax.set_xticks(x)
    ax.set_xticklabels(["Standard", "Chain of Draft"])
    ax.set_ylabel("Mean cumulative GT")
    set_error_ylim(
        ax,
    ...
    ax.text(
        0.03, 0.05,
        f"$\\hat{{\\Delta}}={boot['point_estimate']:.1f}$ GT\n"
