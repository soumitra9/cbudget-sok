...
% Mandatory TMLR style (JmlrOrg/tmlr-style-file). Default is anonymized
% double-blind submission; use \usepackage[accepted]{tmlr} for the camera-ready
% or \usepackage[preprint]{tmlr} for a non-anonymous arXiv version.
\usepackage{tmlr}
...
% Double-blind toggle: when true, identifying links, the reproducibility-archive
% citation, and the author-contributions statement are suppressed for review.
% Set \anonfalse for the camera-ready.
\newif\ifanon\anontrue

\newcommand{\tabtworow}[2]{\begin{tabular}[t]{@{}>{\raggedright\arraybackslash}p{1.75cm}@{}}#1\\[-2pt]{\small #2}\end{tabular}}

\title{Context as a Budget: A Taxonomy, an Accounting Framework, and a Measurement Audit for Composing Token-Efficiency Interventions in {LLM} Agents}

\author{%
  \name Soumitra Mehrotra \email soumitra.mehrotra9@gmail.com \\
  \addr San Francisco, USA
  \AND
  \name Shubhender Singh \\
  \addr San Francisco, USA
  \AND
  \name Saurabh Aggarwal \\
  \addr San Francisco, USA
}
...
{\footnotesize\raggedright $^{\mathrm{a}}$Self-reported or vendor documentation; not independently peer-reviewed. Cat.\ = intervention-point category (Section~\ref{sec:taxonomy}); OUT = excluded boundary case (Section~\ref{sec:background}). Failure modes include reported and analytically inferred risks; provenance is provided in the extraction sheet.\par}
\end{sidewaystable}
...
\bottomrule
\end{tabularx}
{\footnotesize\raggedright $n=12$ per cell (6 tasks $\times$ 2 seeds). Model: Qwen2.5-Coder-7B-Instruct (rev.~c03e6d3). Compaction trigger $\tau=4{,}096$ tokens.\par}
\end{table}
...
\bottomrule
\end{tabularx}
{\footnotesize\raggedright $n=12$ per cell, twelve (task, seed) blocks. PT/turn and PT for compaction are the same effect in different units and are not independent tests.\par}
\end{table}
...
\bottomrule
\end{tabularx}
{\footnotesize\raggedright Present design: $n=12$ per cell.\par}
\end{table}
...
\ifanon
The acknowledgments are omitted to preserve double-blind review and will be added in the camera-ready version.
\else
The authors did not receive support from any organization for the submitted work and have no competing interests to disclose. Ethics approval, consent to participate, and consent for publication are not applicable.
\fi

\subsubsection*{Data and code availability}
\ifanon
The aggregated run table, the significance-audit artifacts that produce the central exhibit (Table~\ref{tab:audit}), the analysis and figure code, and the frozen preregistered hypothesis files whose SHA-256 hashes are listed in Appendix~\ref{app:prereg} are provided as anonymized supplementary material for review. A public archive (Zenodo DOI and source repository) will be linked in the camera-ready version. Raw agent run traces are not archived owing to their size and environment-specific paths.
\else
The aggregated run table, the significance-audit artifacts that produce the central exhibit (Table~\ref{tab:audit}), the analysis and figure-generation code, and the frozen preregistered hypothesis files whose SHA-256 hashes are listed in Appendix~\ref{app:prereg} are archived at Zenodo, \url{https://doi.org/10.5281/zenodo.21715347}~\citep{cab_dataset2026}, and mirrored in a public source repository. The confirmatory estimands and the full measurement audit of Section~\ref{sec:audit} reproduce byte-for-byte from the raw run traces through the archived extraction and bootstrap pipeline. Raw agent run traces are not archived owing to their size and environment-specific paths.
\fi
...
\textbf{Blocking:} E1 (RTK-compaction): 6 tasks $\times$ 2 seeds (17, 52). E2 (RTK-CoD): 6 tasks $\times$ 6 seeds (7, 13, 17, 42, 52, 99). Analysis scripts and preregistered hypothesis files are provided in the \ifanon anonymized supplementary material\else reproducibility archive~\citep{cab_dataset2026}\fi.
...
The frozen hypothesis files themselves are provided in the \ifanon anonymized supplementary material\else reproducibility archive~\citep{cab_dataset2026}\fi, allowing independent verification of these hashes.
